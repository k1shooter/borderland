...
function setPhase(session, phase, seconds, chatEnabled = false, clearSubmissions = true) {
  session.phase = phase;
  session.phaseId = (session.phaseId || 0) + 1;
  session.phaseToken = phaseToken();
  session.phaseStartedAt = now();
  session.deadline = seconds ? now() + seconds * 1000 : null;
  session.chatEnabled = chatEnabled;
  if (clearSubmissions) {
    session.submissions = {};
    if (!session.privateData.submissionPhase) session.privateData.submissionPhase = {};
  }
}
...
function resolvePhysical(session) {
  ...
  const inputStats = session.privateData.inputStats || {};
  const playerIds = livingIds(session);
  const scores = playerIds.map((playerId) => ({
    playerId,
    m: sanitizePhysicalMetrics(session.cardCode, metrics[playerId] || {}, inputStats[playerId] || {}),
  }));
  let winners = [];
...
function resolveMaskBid(session) {
  ...
  bids.forEach((row) => {
    session.privateData.coins[row.id] = Math.max(0, (session.privateData.coins[row.id] || 0) - row.bid);
  });
  const roleById = {};
  const remaining = [...bids];
  const wolfCandidates = remaining.filter((b) => b.mask === 'Wolf').sort((a, b) => b.bid - a.bid);
  if (wolfCandidates[0]) {
    const topBid = wolfCandidates[0].bid;
    const topCandidates = wolfCandidates.filter((item) => item.bid === topBid);
    roleById[choice(session.rng, topCandidates).id] = 'Wolf';
  }
  ...
  if (foxCandidates[0]) {
    const topBid = foxCandidates[0].bid;
    const topCandidates = foxCandidates.filter((item) => item.bid === topBid);
    roleById[choice(session.rng, topCandidates).id] = 'Fox';
  }
...
const submissionSchemas = {
  'sync-press': z.object({ pressed: z.boolean().optional() }),
  'card-pass-sum': z.object({ cardIndex: z.number().int().min(0).max(32) }),
  'relay-inference': z.object({ answer: z.string().max(200).optional().default('') }),
  'binary-balance': z.object({
    question: z.string().max(180).optional(),
    answer: z.enum(['yes', 'no']).optional(),
  }),
  'map-consensus': z.object({ cell: z.union([z.string(), z.number()]).optional() }),
  'team-contribution': z.object({ amount: z.number().int().min(0).max(30).optional() }),
  'sequence-assembly': z.object({ order: z.array(z.union([z.string(), z.number()])).optional() }),
  'load-sharing': z.object({ load: z.number().int().min(0).max(8).optional() }),
  'trust-chain': z.object({ distribution: z.record(z.string(), z.number()).optional() }),
  'route-consensus': z.object({ route: z.enum(['A', 'B', 'C']).optional() }),
  'puzzle-codebreak': z.object({ digits: z.array(z.number().int().min(1).max(6)).length(4).optional() }),
  'puzzle-order': z.object({ order: z.array(z.union([z.string(), z.number()])).optional() }),
  'estimate-auction': z.object({
    estimate: z.number().int().min(0).max(20).optional(),
    risk: z.number().int().min(1).max(5).optional(),
  }),
  'grid-path': z.object({ path: z.string().max(50).optional() }),
  'puzzle-truth-ledger': z.object({ liars: z.array(z.string().max(1)).max(4).optional() }),
  'weighted-average': z.object({
    value: z.number().int().min(1).max(9).optional(),
    weight: z.number().int().min(1).max(3).optional(),
  }),
  'number-bingo': z.object({
    choices: z.array(z.number().int().min(1).max(9)).max(4).optional(),
    number: z.number().int().min(1).max(9).optional(),
  }),
  'countdown-guess': z.object({ guess: z.number().int().min(1).max(100).optional() }),
  'liar-counting': z.object({
    bag: z.record(z.string(), z.number()).optional(),
    mode: z.string().max(20).optional(),
    animal: z.string().max(20).optional(),
    count: z.number().int().min(0).max(12).optional(),
    action: z.string().max(20).optional(),
  }),
  'circuit-solve': z.object({ switches: z.record(z.string(), z.union([z.number(), z.string()])).optional() }),
  'text-vote': z.object({
    text: z.string().max(280).optional(),
    target: z.string().max(64).optional(),
  }),
  'hidden-role-rps': z.object({ play: z.enum(['rock', 'paper', 'scissors']).optional() }),
  'gift-poison': z.object({
    target: z.string().max(64).optional(),
    item: z.enum(['poison', 'antidote', 'empty']).optional(),
  }),
  'anonymous-vote': z.object({ target: z.string().max(64).optional() }),
  'pair-trust': z.object({ action: z.enum(['catch', 'guard', 'cut']).optional() }),
  'pot-split': z.object({ action: z.enum(['share', 'steal', 'burn']).optional() }),
  'mask-dealer': z.object({
    mask: z.enum(['Wolf', 'Fox', 'Sheep']).optional(),
    bid: z.number().int().min(0).max(100).optional(),
    target: z.string().max(64).optional(),
  }),
  'confession-market': z.object({
    claimIndex: z.number().int().min(0).max(1).optional(),
    target: z.string().max(64).optional(),
    stance: z.enum(['trust', 'doubt']).optional(),
  }),
  'knife-relay': z.object({ action: z.enum(['left', 'right', 'hold']).optional() }),
  'blood-contract': z.object({
    target: z.string().max(64).optional(),
    mode: z.enum(['contract', 'betray']).optional(),
  }),
};

const physicalMetricsSchema = z.object({
  holdRatio: z.number().min(0).max(1).optional(),
  maxOutMs: z.number().min(0).max(999999).optional(),
  accuracy: z.number().min(0).max(1).optional(),
  combo: z.number().int().min(0).max(999).optional(),
  hits: z.number().int().min(0).max(9999).optional(),
  misses: z.number().int().min(0).max(9999).optional(),
  distance: z.number().min(0).max(999999).optional(),
  crashes: z.number().int().min(0).max(9999).optional(),
  score: z.number().min(-99999).max(999999).optional(),
  segmentsCleared: z.number().int().min(0).max(999).optional(),
  bandRatio: z.number().min(0).max(1).optional(),
  peak: z.number().min(0).max(1).optional(),
  cores: z.number().int().min(0).max(99).optional(),
  health: z.number().min(0).max(100).optional(),
  stability: z.number().min(0).max(100).optional(),
  complete: z.boolean().optional(),
  charge: z.number().min(0).max(1000).optional(),
  overheat: z.number().int().min(0).max(999).optional(),
  totalScore: z.number().min(-99999).max(999999).optional(),
  stageScores: z.array(z.number()).optional(),
});

function phaseToken() {
  return createId('phase');
}

function validatePayloadByEngine(engine, payload) {
  const schema = submissionSchemas[engine];
...
function normalizeEnvelope(session, playerId, envelope, options = {}) {
  const allowMulti = !!options.allowMulti;
  const skipEngineValidation = !!options.skipEngineValidation;
  const player = session.playerMap[playerId];
  if (!player) throw new Error('not room member');
  if (!isAlive(session, playerId)) throw new Error('player already dead');

  const wrapped =
    envelope &&
    typeof envelope === 'object' &&
    Object.prototype.hasOwnProperty.call(envelope, 'payload');
  if (!wrapped && !player.isBot && session.privateData?.strictEnvelope) {
    throw new Error('envelope required');
  }
  const raw = wrapped ? envelope : { payload: envelope || {} };

  const currentPhaseId = session.phaseId || 0;
  const givenPhaseId = Number.isInteger(raw.phaseId) ? raw.phaseId : currentPhaseId;
  if (givenPhaseId !== currentPhaseId) throw new Error('phase mismatch');

  const givenToken = typeof raw.phaseToken === 'string' ? raw.phaseToken : '';
  if (
    wrapped &&
    !player.isBot &&
    session.privateData?.strictEnvelope &&
    givenToken !== session.phaseToken
  ) {
    throw new Error('phase token mismatch');
  }

  const seq = parseInt(raw.seq, 10) || 0;
  if (wrapped) {
    if (!session.privateData.seqByPlayer) session.privateData.seqByPlayer = {};
    const prevSeq = session.privateData.seqByPlayer[playerId] || 0;
    if (seq && seq <= prevSeq) throw new Error('replayed sequence');
    if (seq) session.privateData.seqByPlayer[playerId] = seq;
  }

  if (!allowMulti) {
    const alreadySubmitted = !!session.submissions[playerId];
    if (alreadySubmitted) {
      throw new Error('already submitted');
    }
  }

  const payload = skipEngineValidation
    ? (raw.payload || {})
    : validatePayloadByEngine(session.engine, raw.payload || {});
  return { payload, phaseId: givenPhaseId, seq };
}

function markSubmission(session, playerId, submission) {
  session.submissions[playerId] = submission;
  if (!session.privateData.submissionPhase) session.privateData.submissionPhase = {};
  session.privateData.submissionPhase[playerId] = session.phaseId || 0;
}

const inputFrameSchema = z.array(
  z.object({
    t: z.number().int().min(0).max(10_000_000),
...
function sanitizePhysicalMetrics(cardCode, rawMetrics, inputStats) {
  const parsed = physicalMetricsSchema.safeParse(rawMetrics || {});
  const metrics = parsed.success ? parsed.data : {};
  const stats = inputStats || { eventCount: 0, activeMs: 0 };
  const minEvents = cardCode === 'S4' ? 6 : 12;
  const minActiveMs = 1500;
  if ((stats.eventCount || 0) < minEvents || (stats.activeMs || 0) < minActiveMs) {
    return {};
  }
  return metrics;
}
...
function isPhysicalEngine(engine) {
  return String(engine).startsWith('physical-');
}

function allowsMultiSubmission(engine) {
  return engine === 'puzzle-codebreak';
}

function submitInput(session, playerId, envelope = {}) {
  if (session.status === 'complete') return { accepted: 0 };
  if (!isPhysicalEngine(session.engine)) return { accepted: 0 };
  const normalized = normalizeEnvelope(session, playerId, envelope, {
    allowMulti: true,
    skipEngineValidation: true,
  });
  const parsed = inputFrameSchema.safeParse(normalized.payload.frames || []);
  if (!parsed.success) throw new Error('invalid input frames');
  const frames = parsed.data;
  if (!session.privateData.inputStats) session.privateData.inputStats = {};
  if (!session.privateData.inputStats[playerId]) {
    session.privateData.inputStats[playerId] = {
      eventCount: 0,
      activeMs: 0,
      spamBursts: 0,
      lastAt: null,
  ...
  const stats = session.privateData.inputStats[playerId];
  let previousAt = typeof stats.lastAt === 'number' ? stats.lastAt : null;
  frames.forEach((frame) => {
    if (previousAt != null) {
      const delta = clamp(frame.t - previousAt, 0, 300);
      if (delta > 0) stats.activeMs += delta;
      if (delta > 0 && delta < 15) stats.spamBursts += 1;
    }
    previousAt = frame.t;
  });
  if (frames.length > 0) stats.lastAt = previousAt;
  stats.eventCount += frames.length;
  return { accepted: frames.length };
}

function submit(session, playerId, envelope = {}) {
  if (session.status === 'complete') return;
  const allowMulti = allowsMultiSubmission(session.engine);
  const normalized = normalizeEnvelope(session, playerId, envelope, {
    allowMulti,
  });
  const payload = normalized.payload || {};
  if (isPhysicalEngine(session.engine)) {
    const stats = session.privateData.inputStats?.[playerId] || {};
    const safeMetrics = sanitizePhysicalMetrics(
      session.cardCode,
      payload.metrics || {},
      stats
    );
    if (payload.done || Object.keys(safeMetrics).length > 0) {
      if (!session.privateData.metrics) session.privateData.metrics = {};
      session.privateData.metrics[playerId] = safeMetrics;
      markSubmission(session, playerId, { done: true });
    }
  ...
  switch (session.engine) {
    case 'sync-press':
      markSubmission(session, playerId, { serverAt: now(), pressed: true });
      if (livingIds(session).every((id) => session.submissions[id])) resolveSyncPress(session);
      return;
    case 'card-pass-sum':
      markSubmission(session, playerId, { cardIndex: payload.cardIndex });
      if (livingIds(session).every((id) => session.submissions[id])) applyCardPass(session);
      return;
    case 'relay-inference':
      markSubmission(session, playerId, { answer: payload.answer });
      if (livingIds(session).every((id) => session.submissions[id])) resolveRelayInference(session);
      return;
    case 'binary-balance':
      if (session.phase === 'ask') {
        const askerId = currentAsker(session);
        if (playerId !== askerId) throw new Error('not asker turn');
        markSubmission(session, playerId, {
          question: String(
            payload.question || choice(session.rng, YES_NO_QUESTION_BANK)
          ).slice(0, 180),
        });
        session.publicData.question = session.submissions[playerId].question;
        setPhase(session, 'answer', 25, false);
      } else {
        markSubmission(session, playerId, {
          answer: payload.answer === 'no' ? 'no' : 'yes',
        });
        const askerId = currentAsker(session);
        const others = session.players
          .map((p) => p.id)
          .filter((id) => id !== askerId && !session.deadIds.includes(id));
        if (others.every((id) => session.submissions[id])) {
          advanceBinaryBalance(session);
        }
      }
      return;
    case 'map-consensus':
      markSubmission(session, playerId, { cell: payload.cell });
      if (livingIds(session).every((id) => session.submissions[id])) resolveMapConsensus(session);
      return;
    case 'team-contribution':
      markSubmission(session, playerId, { amount: payload.amount });
      if (livingIds(session).every((id) => session.submissions[id])) applyContributionRound(session);
      return;
    case 'sequence-assembly':
      markSubmission(session, playerId, { order: payload.order });
      if (livingIds(session).every((id) => session.submissions[id])) resolveSequenceAssembly(session);
      return;
    case 'load-sharing':
      markSubmission(session, playerId, { load: payload.load });
      if (livingIds(session).every((id) => session.submissions[id])) resolveLoadSharingWave(session);
      return;
    case 'trust-chain':
      markSubmission(session, playerId, { distribution: payload.distribution || {} });
      if (livingIds(session).every((id) => session.submissions[id])) resolveTrustChainWave(session);
      return;
    case 'route-consensus':
      markSubmission(session, playerId, { route: payload.route });
    ...
    case 'number-bingo':
      if (session.phase === 'bingo-setup') {
        ...
        markSubmission(session, playerId, { number: payload.number });
        if (livingIds(session).every((id) => session.submissions[id])) applyBingoTurn(session);
      }
      return;
    case 'countdown-guess':
      if (playerId !== currentBombHolder(session)) throw new Error('not holder turn');
      markSubmission(session, playerId, { guess: payload.guess });
      applyCountdownGuess(session);
      return;
    case 'liar-counting':
      if (session.phase === 'animal-select') {
        markSubmission(session, playerId, { bag: payload.bag || {} });
        if (livingIds(session).every((id) => session.submissions[id])) {
          livingIds(session).forEach((id) => {
            session.privateData.animalBags[id] = normalizeAnimalBag(
              session.submissions[id].bag
            );
          });
    ...
    case 'anonymous-vote':
      ...
      return;
    case 'pair-trust':
      markSubmission(session, playerId, { action: payload.action });
    ...
    case 'mask-dealer':
      if (session.phase === 'mask-bid') {
        markSubmission(session, playerId, { mask: payload.mask, bid: payload.bid });
    ...
    case 'confession-market':
      if (session.phase === 'confession-publish') {
        markSubmission(session, playerId, { claimIndex: payload.claimIndex });
        if (livingIds(session).every((id) => session.submissions[id])) resolveConfessionPublish(session);
      } else {
        markSubmission(session, playerId, { target: payload.target, stance: payload.stance });
    ...
    case 'knife-relay':
      ...
      return;
    case 'blood-contract':
      markSubmission(session, playerId, { target: payload.target, mode: payload.mode });
...
function pendingIdsForDeadline(session) {
  ...
  if (session.engine === 'liar-counting' && session.phase === 'animal-claim') {
    const active = activeSetPlayers(session);
    if (!active.length) return [];
    return [active[session.publicData.turnIndex % active.length]];
  }
  return livingIds(session).filter(
    (id) =>
      !session.submissions[id] ||
      Object.keys(session.submissions[id]).length === 0
  );
}
...
function onDeadline(session) {
  ...
  const targets = pendingIdsForDeadline(session);
  targets.forEach((id) => {
    if (session.status === 'complete') return;
    ...
    submit(session, id, {
      ...
      phaseToken: session.phaseToken,
      seq: (session.privateData.seqByPlayer?.[id] || 0) + 1,
      payload,
    });
  });
  if (session.status !== 'complete' && session.deadline && now() > session.deadline) {
    if (session.engine === 'puzzle-codebreak') {
      ...
      killPlayers(session, livingIds(session).filter((id) => !winners.includes(id)), '?? ??');
      finishSession(session, winners, '?? ?? ? ?? ??');
    }
  }
}

function autoSubmitBots(session) {
  if (session.status === 'complete') return;
  const limit = 20;
  for (let step = 0; step < limit; step += 1) {
    let moved = false;
    const botIds = livingIds(session).filter(
      (id) => session.playerMap[id]?.isBot && !session.submissions[id]
    );
    botIds.forEach((id) => {
      if (session.status === 'complete') return;
      const view = getView(session, id);
      ...
      try {
        ...
        // Not this bot's turn or invalid state.
      }
    });
    if (!moved || session.status === 'complete') break;
  }
}

function adminSkip(session) {
  if (session.status === 'complete') return;
  onDeadline(session);
}
